...
El concepto de routing involucra lo relacionado con páginas y navegación. A diferencia de lo que ocurre cuando trabajamos con create-react-app por ejemplo, y en Next.js tenemos una solución para no tener que preocuparnos por eso.  Cada página en Next.js está representada por un componente React.
...
Como consecuencia de esto al hacer un *request* a `http://localhost:3000/api/user/:id` obtendremos un json con el id.simplificar la notación y en lugar de tener `router.query.productId` usar `productId` podemos utilizar *nested object destructuring* 
...
## Enlazando Páginas
A la hora de enlazar páginas entre sí u.
